...
const CRMPage = () => {
  ...
  useEffect(() => {
    filterClients();
  }, [clients, searchTerm, otwFilter, chatStatusFilter]);
...
if (chatStatusFilter !== 'all') {      filtered = filtered.filter((client) => (client.status || 'TODO') === chatStatusFilter);    }
      ...
      <div className="filters">
        ...
        >
          ...
          <select
            ...
            className="status-filter"
          >
            {chatStatusCategories.map(category => (
              <option key={category} value={category}>
                {category === 'all' ? 'All Chat Status' : category}
              </option>
            ))}
          </select>
      </div>
